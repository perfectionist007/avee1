  ...
  if (isMobile === null) return null;
    ...
    return (
          <header className={`sticky top-0 z-50 border-b h-16 text-[15px] backdrop-blur supports-[backdrop-filter]:bg-background/60 inset-0 flex-center ${isDarkMode ? 'bg-black' : 'bg-white'}`}>
            <nav className={`px-3.5 flex-center-between w-full mx-auto ${isDarkMode ? 'text-white' : 'text-black'} `}>
            <Link href="/">
...
{/*                 <img src={Logo} alt="" className="size-8" />*/} 
                    ...
                    priority
                    />
                <h3 className="text-xl font-semibold">Aveenir</h3>
              </div>
            </Link>
            {isMobile ? (
              <MobMenu Menus={Menus} />
              ...
              <div className="flex-center gap-x-5">
                <button
                  aria-label="sign-in"
                  className="bg-white/5 z-[999] relative px-3 py-1.5 shadow rounded-xl flex-center text-xl"
                >
                  Sign In
                </button>
                <button 
